...
export default function Page() {
  ...
  const { 
    data: responseData, // 이름 변경: article -> responseData
    isLoading, 
    isError,
    isFetching,
  ...
  if (isLoading || (isFetching && !article))
    return <div className="flex justify-center p-20">로딩 중...</div>;
  if (isError)
    return (
    ...
    <div className="mx-auto">
      ...
      <div className="mx-auto w-[920px] bg-white p-20 min-h-screen">
        ...
        <div className="mt-1 flex space-x-2">
          {/* 태그가 있다면 렌더링 */}
          {article.mcps &&
            Object.keys(article.mcps).map((mcpKey: string, index: number) => (
              <span
                key={index}
                className="text-[#555555] bg-[#EDEDED] px-2 py-1 rounded-full text-sm"
              >
                {mcpKey}
              </span>
            ))}
        ...
        {/* 추천/공유 버튼 그룹 - isLiked 사용 수정 */}
        <div className="mt-5 flex justify-end space-x-4">
          <button
            className={`w-24 px-4 py-2 rounded-full border ${
              isLiked // article?.isLiked 대신 isLiked 사용
